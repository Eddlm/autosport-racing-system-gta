...
- **XML supplier files**: the `<Model>` InnerText is the **signed-int hash** (the cache key); the `ModelName` attribute is the friendly model name, not the display name. Power/grip/top-speed are **not** in the XML — they come from the game at runtime via the model natives. The model-level natives take an **int model hash**, not a `Model` object — passing a `Model` crashes.
