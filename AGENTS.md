...
- **Steering limit** (`ApplySteerLimits`, only when steer and yaw share sign): **an empirical peak-memory system** — it learns the steer angle that produced the best *measured* lateral G (`AverageAcceleration` projected on the car's right vector, NaN-guarded). Utilization is measured against that **remembered peak** (not the declared grip): near the peak → exploit (limit = remembered steer + a small margin, with a floor), well below it → explore (full steer-lock authority). Hysteresis latch avoids dithering; the memory decays so stale peaks let go. **Declared `CurrentMechanicalGrip` is demoted to a sanity cap only**: records above it ×1.1 are rejected as impact/bump spikes (a collision impulse once ratcheted the memory to an impossible peak and locked the system into explore). Declared grip often reads ~2× what the car really achieves, so it is not usable as the utilization reference. The two previous limit systems (grip-saturation formula × throttle multiplier; speed lerp from steer lock to `|slide| + 4°`) are commented out above it for reference. Hitting the limit still caps `MaxThrottle` (pegged → throttle cap, 10° past → hard 1/s throttle cut). TRlat/declared curves are **not trusted** as grip targets.
