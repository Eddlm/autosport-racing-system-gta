...
- **Steering limit** (`ApplySteerLimits`, only when steer and yaw share sign): **an empirical peak-memory system** — it learns the steer angle that produced the best *measured* lateral G. **Everything in this block is now in Gs** (latG is `AverageAcceleration·right ÷ 9.8f`, `_bestLatG` is stored in Gs, and the declared grip ceiling is just `CurrentMechanicalGrip` — a grip-from-native value already in Gs, NOT multiplied by gravity). A lingering 9.8× unit bug (m/s² vs Gs mixed in the ceiling and debug, showing ~21 "G") was the source of endless confusion — always keep this block in Gs. Utilization is measured against the remembered peak (not declared grip): near the peak → exploit (limit = remembered steer + a small margin, with a floor), well below it → explore (full steer-lock authority). Hysteresis latch avoids dithering. **Rise and decay of the peak memory are both exponential and frame-independent** (`exp(-dt/τ)`), decay deliberately **very fast** (stale ~1 G error mostly corrected in ~0.3 s) so the memory resets between corners and every corner reprobes — accepted by design. Declared grip is only the sanity cap that rejects impact/bump spikes (records above it ×1.1). The two previous limit systems (grip-saturation formula × throttle multiplier; speed lerp from steer lock to `|slide| + 4°`) are commented out above it for reference. Hitting the limit still caps `MaxThrottle` (pegged → throttle cap, 10° past → hard 1/s throttle cut). TRlat/declared curves are **not trusted** as grip targets.
