...
- **The reference can be either the raw geometric command or the speed-limited one, and in normal driving the two are nearly the same number — verified, and it explains more than it seems.** The variant that references the **post-limit, pre-slew** steer (`_limitedSteerDeg`, captured at the end of `ApplySteerLimits` and read one frame later because the limiter runs *after* `ComputeSteering` and `TranslateSteerToInput` overwrites its output in the same tick) was built and driven: **behaviour is very similar to the raw reference.** The reason is arithmetic, not coincidence — the limit is a flat `TRlat × 0.33` above the ramp speed, which is ≈3.3–6.6° for TRlat 10–20, while the pure-pursuit command in a normal corner is only **~1.5°** (30 m/s, 100 m radius, `d ≈ 15 m`, `e ≈ 4.3°`) and the geometry keeps it small as speed rises because `d` grows with speed. So the limiter **rarely clamps**, and the two references are the same value most of the time. **This means the earlier prediction that "D goes blind at speed, because the limiter prevents the car reaching `ω_des`" was largely untested by normal driving** — the blindness needs the limiter to actually be biting. **Where the two would genuinely diverge: a low-grip car** (TRlat ≈ 6 gives a ceiling ≈ 2°, the same order as the command itself), **or a transient with the aim point swung abeam** — which is exactly the low-grip case already on the release-gate list. **Test the difference there, not on a grippy car.** Reverting to the raw reference is one line (substitute `pSteer` in the yaw-rate computation); **the veto tests `pSteer` either way, deliberately**, so the reference is the only changed variable.
