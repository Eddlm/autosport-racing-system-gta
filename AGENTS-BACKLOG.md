...
When `ApplySteerLimits` actually cuts the steer (`_steerLimitedThisFrame`), `MaxThrottle` drops at a slow rate with a floor and recovers once the limiter disengages — **but only while the limiter has real authority** (the limited steer below a fraction of the steering lock). Near lock the steer is still trimmed and throttle is left alone on purpose: the limiter reaches full lock exactly where steering is free, and starving throttle there strangled recovering cars, which need pedal authority to drive away while they steer back toward the track. Consequence: the cut is a high-speed-only device — the allowance crosses the gate roughly a quarter of the way up the speed ramp. **TODO: revisit the cut rate** — tuned to "feels right", with no grounded justification.
