...
## Key ownership — one setting, two files (found 2026-10, NOT fixed)

**`Laps` has two homes and the menu's knob is not the one the race reads.** The Race menu's `Laps` item writes `Menu-Race.ini` (`RaceMenuStore.Set("Laps", …)`) and the load migrates `Options.ini`'s value into it, but every *race-logic* reader still reads `SettingsFile` (= `Options.ini`): `OnTick` (finish/timed block), the reward seed, and `Racer.cs` twice (progress total + the lap-crossing finish test). On the live install that is `Menu-Race.ini` **Laps = 5** vs `Options.ini` **Laps = 3** — the menu says 5, the race runs 3, and changing the menu item changes nothing. The repair schema cannot paper over this: it declares `Laps` as a `Menu-Race.ini` key (that is where the writer lives, and AGENTS.md's "per-session knobs stay in the Race menu" says the menu is the intended owner), so the readers are the outliers. Recommended fix: repoint those four readers at `RaceMenuStore.GetInt("Laps", 5)`, after which `Options.ini`'s `Laps` survives only as the one-time migration source. Not applied because it changes race length for an existing install (5 laps instead of 3) and the user is the verification gatekeeper.
