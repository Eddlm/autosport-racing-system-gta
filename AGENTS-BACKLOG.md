...
## Race-start cost and the "tuning" hypothesis (2026-10, unverified)

Two incidents in one session (textures stopped streaming minutes after `arsupdroute`; then a hard wedge during race setup where **even SHVDN's own script-reload did nothing** — meaning no script tick was happening at all, i.e. the block is game/native level, not ARS logic) share one plausible cause: streaming/memory exhaustion accumulated over a session. `AITuningLevel` is the strongest candidate found so far:

- `RandomTuning` walks **every** `VehicleMod` slot, and for each available one calls `Script.Wait(30)` then `SetMod` — so a single car streams ~20-30 mod parts (bumpers, spoilers, wheels, liveries…), each a model the game must load. A 12-car grid is 200+ mod parts per race, plus ~0.6-1 s of script-thread time **per car**, all inside the race start. Level 2/3 add performance mods + turbo; level 3 also sets `EnginePowerMultiplier`.
- **`AITuningLevel = 0` does not switch tuning off entirely**: `ApplyCarAppearance` (called per car from `LoadGrid`) runs the full `RandomTuning(car, true, true, true, true, false)` for any vehicle whose supplier XML carries the **tuner** discipline tag — independent of the tuning level. A clean test therefore has to account for the fleet's tags.
- It is **not** the immediate trigger of the black-screen wedge (in that run the log stopped before `Loading vehicle models`, while tuning runs later in `SetupRace`) — but it fits the session-degradation pattern both incidents share.
- Experiment for the user, no code change needed: run a few races with `AI Settings -> AITuningLevel = 0` and watch for recurrence. If it still recurs, the tuner-tag path is the remaining tuning source and would want gating or an opt-in.
- Related release note: `AITuningLevel` ships with a default (currently 1) in the declared schema — if the correlation holds, the shipped default is a release decision, like the other Dist defaults.
