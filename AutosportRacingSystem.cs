...
namespace ARS
{
        ...
        public static Dictionary<Options, bool> _debugToggles = new Dictionary<Options, bool>()
    {
        ...
        public static int _optionHovered = 0;
        public static List<Options> _optionsList = new List<Options>();
        ...
        string FormatEnumName(string t)
        {
        ...
        {
            _optionsList.Clear();
            _optionsList.AddRange(options);
        }

        string GetOptionName(Options option)
        {
            return FormatEnumName(option.ToString());
        }

        string GetOptionValueSuffix(Options option)
        {
            if (_debugToggles.ContainsKey(option)) return _debugToggles[option] ? " [~g~X~w~]" : " [ ]";
            return "";
        }
        ...
        void StartFromMenu()
        {
            ...
            XmlDocument f = LoadTrackFile(_filteredTracks[_trackListPos]);

            if (_debugToggles[Options.UseNearbyCars])
            {
            ...
            try
            {
                LoadGrid(_disciplineFilter, (int)Clamp(_intendedOpponents - _racers.Count, 0, _gridPositions.Count));
            }
            catch (Exception ex)
            {
            ...
            if (CanWeUse(Game.Player.Character.CurrentVehicle)) _helpMessages.Add("Rev up your engine to start the countdown.");
        }
            ...
            MenuItemDefinitions[Options.StartRace] = new MenuItemDefinition
            {
                OnAccept = () =>
                {
                    if (_routeNodes.Count == 0)
                    {
                        ...
                        XmlDocument f = LoadTrackFile(_filteredTracks[0]);
                        LoadTrack(f);
                    }
        ...
        {
            if (_debugToggles.ContainsKey(option)) return true;
        ...
        void HandleMenuAccept(Options selected)
        {
            ...
            if (_debugToggles.Keys.Contains(selected)) _debugToggles[selected] = !_debugToggles[selected];
        ...
        void HandleMenu()
        {
            ...
            if (_filteredTracks.Count == 0) _trackListPos = 0;
            else if (_trackListPos > _filteredTracks.Count - 1) _trackListPos = _filteredTracks.Count - 1;

            Game.DisableControlThisFrame(2, GTA.Control.Phone);

            if (_optionHovered > _optionsList.Count - 1) _optionHovered = _optionsList.Count - 1;
            if (_optionHovered < 0) _optionHovered = _optionsList.Count - 1;

            float zoffset = 0;
            foreach (Options o in _optionsList)
            {
                string line = (o == _optionsList[_optionHovered] ? "~b~> " : "") + GetMenuItemName(o) + "~w~" + GetMenuItemValueSuffix(o);
                DrawText(new Vector2(0.02f, 0.2f + zoffset), line, Color.White, DrawTextFont.Default, DrawTextAlign.Left, 0.5f);
                zoffset += 0.04f;
            }

            if (Game.IsControlJustPressed(2, GTA.Control.FrontendDown)) _optionHovered++;
            if (Game.IsControlJustPressed(2, GTA.Control.FrontendUp)) _optionHovered--;

            if (_optionHovered < 0) _optionHovered = _optionsList.Count - 1;
            if (_optionHovered > _optionsList.Count - 1) _optionHovered = 0;

            Options oSelected = _optionsList[_optionHovered];
        ...
        void OnTick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (positions.Count > 0 && _optionsList.Count == 0)
                {
                    float z = 0.15f;
                    float scale = 0.4f;
